...
const FormRenderer = ({ node, onFormSubmit, disabled, language, slots, onGridRowClick }) => {
  // ... (기존 FormRenderer 코드 유지) ...
    ...
    const handleDateInputClick = () => {
        try { dateInputRef.current?.showPicker(); } catch (error) { console.error("Failed to show date picker:", error); }
    };

    // 슬롯 데이터를 사용하는 그리드 요소가 있는지 확인
    const hasSlotBoundGrid = node.data.elements?.some(
        (el) => el.type === "grid" && el.optionsSlot && Array.isArray(slots[el.optionsSlot]) && slots[el.optionsSlot].length > 0 && typeof slots[el.optionsSlot][0] === "object" && slots[el.optionsSlot][0] !== null
    );
            ...
            {node.data.elements?.map((el) => {
                ...
                if (el.type === "dropbox") {
                    if (el.optionsSlot && Array.isArray(slots[el.optionsSlot])) {
                        dropboxOptions = slots[el.optionsSlot].map(opt => typeof opt === 'object' && opt !== null ? JSON.stringify(opt) : String(opt));
                        ...
                        {el.type === "grid" ? (
                             (() => {
                                ...
                                if (hasSlotData && typeof gridDataFromSlot[0] === 'object' && gridDataFromSlot[0] !== null && !Array.isArray(gridDataFromSlot[0])) {
                                    ...
                                    const filteredKeys = el.hideNullColumns ? originalDisplayKeys.filter(key => gridDataFromSlot.some(obj => obj[key] !== null && obj[key] !== undefined && obj[key] !== '')) : originalDisplayKeys;
                                    if (filteredKeys.length === 0) return <div>{el.hideNullColumns ? "All columns hidden." : "No data columns found."}</div>;
                                    const columnWidths = filteredKeys.reduce((acc, key) => {
                                        const headerLength = interpolateMessage(key, slots).length;
                                        const maxLength = gridDataFromSlot.reduce((max, obj) => Math.max(max, String(interpolateMessage(obj[key] || '', slots)).length), 0);
                                        acc[key] = Math.max(5, Math.max(headerLength, maxLength) + 2);
                                        return acc;
                                    }, {});
                                    return (
                                        <div style={{ overflowX: 'auto', width: '100%' }}>
                                            <table className={styles.formGridTable} style={{ tableLayout: 'auto' }}>
                                                <thead><tr>{filteredKeys.map(key => <th key={key} style={{ minWidth: `${columnWidths[key]}ch`, textAlign: 'left', padding: '10px 12px' }}>{interpolateMessage(key, slots)}</th>)}</tr></thead>
                                                <tbody>{gridDataFromSlot.map((dataObject, index) => <tr key={`${el.id}-${index}`} onClick={() => !disabled && onGridRowClick(el, dataObject)} style={{ cursor: disabled ? 'default' : 'pointer' }}>{filteredKeys.map(key => <td key={key} style={{ minWidth: `${columnWidths[key]}ch`, whiteSpace: 'nowrap' }}>{interpolateMessage(dataObject[key] || '', slots)}</td>)}</tr>)}</tbody>
                                            </table>
                                        </div>
                                    );
                                } else {
                                    ...
                                    return (
                                        <table className={styles.formGridTable}><tbody>{[...Array(rows)].map((_, r) => <tr key={r}>{[...Array(columns)].map((_, c) => <td key={c}>{interpolateMessage((hasSlotData ? (dataArray[r]?.[c] || '') : (dataArray[r * columns + c] || '')), slots)}</td>)}</tr>)}</tbody></table>
                                    );
                                }
                            })()
                        ) : (
                            <>
                                ...
                                {el.type === "input" && <input className={styles.formInput} type="text" placeholder={interpolateMessage(el.placeholder || '', slots)} value={formData[el.name] ?? interpolateMessage(String(el.defaultValue ?? ''), slots)} onChange={(e) => handleInputChange(el.name, e.target.value)} disabled={disabled} onClick={(e) => e.stopPropagation()} />}
                                {el.type === "date" && <input ref={dateInputRef} className={styles.formInput} type="date" value={formData[el.name] || ""} onChange={(e) => handleInputChange(el.name, e.target.value)} onClick={(e) => { e.stopPropagation(); handleDateInputClick(); }} disabled={disabled} {...dateProps} />}
                                {el.type === "dropbox" && <div className={styles.selectWrapper}><select className={styles.formInput} value={formData[el.name] || ""} onChange={(e) => handleInputChange(el.name, e.target.value)} disabled={disabled} onClick={(e) => e.stopPropagation()}><option value="" disabled>{t("select")}</option>{dropboxOptions.map((opt, idx) => <option key={`${opt}-${idx}`} value={opt}>{interpolateMessage(opt, slots)}</option>)}</select><ArrowDropDownIcon style={{ color: "var(--Gray-07, #5E7599)" }} /></div>}
                                {el.type === "checkbox" && (el.options || []).map((opt) => <div key={opt} onClick={(e) => e.stopPropagation()} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><input type="checkbox" id={`${el.id}-${opt}`} value={opt} checked={(formData[el.name] || []).includes(opt)} onChange={(e) => handleMultiInputChange(el.name, opt, e.target.checked)} disabled={disabled} /><label htmlFor={`${el.id}-${opt}`}>{interpolateMessage(opt, slots)}</label></div>)}
                            </>
                        )}
                    </div>
                );
            })}
             {!hasSlotBoundGrid && !disabled && <button type="submit" className={styles.formSubmitButton} onClick={(e) => e.stopPropagation()}>{t("submit")}</button>}
        </form>
    );
};
...
const ScenarioStatusBadge = ({ status, t, isSelected }) => {
  ...
  switch (status) {
    ...
    case "canceled": text = t("statusCanceled"); statusClass = "canceled"; break;
    default: return null;
  }
...
const PARENT_ORIGIN = process.env.NEXT_PUBLIC_PARENT_ORIGIN || "http://localhost:3000";
      ...
      <div className={styles.header}>
        <div className={styles.headerContent}>
          <span className={styles.headerTitle}>
            {t("scenarioTitle")(interpolateMessage(scenarioId || 'Scenario', activeScenario.slots))}
          </span>
        </div>
              ...
              >
                <div className={ msg.node?.type === 'form' ? styles.scenarioFormMessageContentWrapper : styles.scenarioMessageContentWrapper }>
                    <div className={styles.messageContent}>
                       {msg.node?.type === "form" ? (
                        <FormRenderer node={msg.node} onFormSubmit={handleFormSubmit} disabled={isCompleted} language={language} slots={activeScenario.slots} onGridRowClick={handleGridRowSelected} />
                      ) : msg.node?.type === "iframe" ? (
                        <div className={styles.iframeContainer}>
                          <iframe src={interpolateMessage(msg.node.data.url, activeScenario.slots)} width={msg.node.data.width || "100%"} height={msg.node.data.height || "250"} style={{ border: "none", borderRadius: "8px" }} title="chatbot-iframe"></iframe>
                        </div>
                      ) : msg.node?.type === "link" ? (
                        <div>
                           <a href="#" onClick={(e) => { e.preventDefault(); connectParentLink(interpolateMessage(msg.node.data.content, activeScenario.slots)); }} target="_self" rel="noopener noreferrer" className={styles.linkNode}>
                            {interpolateMessage(msg.node.data.display || msg.node.data.content, activeScenario.slots)}
                             <OpenInNewIcon style={{ marginLeft: '4px', verticalAlign: 'middle', width: '16px', height: '16px' }} />
                          </a>
                        </div>
                      ) : (
                            ...
                            {msg.node.data.replies.map(reply => {
                                ...
                                >
                                    <span className={styles.optionButtonText}>{interpolatedDisplayText}</span>
                                     {interpolatedDisplayText.toLowerCase().includes("link") ? (<OpenInNewIcon style={{ color: 'currentColor' }} />) : (<CheckCircle />)}
                                </button>
                                );
                            })}
                        </div>
           ...
           <div className={styles.messageRow}>
             ...
             <div className={`${styles.message} ${styles.botMessage}`}>
               <div className={styles.scenarioMessageContentWrapper}>
                 <div className={styles.messageContent}>
                   <img src="/images/Loading.gif" alt={t("loading")} style={{ width: "40px", height: "30px" }} />
                 </div>
               </div>
             </div>
           </div>
