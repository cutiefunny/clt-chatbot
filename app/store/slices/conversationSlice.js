...
export const createConversationSlice = (set, get) => ({
  ...
  createNewConversation: async (returnId = false) => {
        ...
        try {
            ...
            // loadConversation 완료 후 ID가 정상 설정되었는지 확인 (방어 코드)
             if (get().currentConversationId !== newConversationId) {
                 ...
                 if (get().currentConversationId !== newConversationId) {
                    console.error("State update race condition: currentConversationId not set after loadConversation.");
                    throw new Error("Failed to properly load the new conversation after creation.");
                 }
            }
            console.log(`New conversation ${newConversationId} created and loaded.`);
    ...
    } else { // 사용자가 없는 경우 (로그아웃 상태 등) UI만 초기화
      set({ currentConversationId: null, expandedConversationId: null });
      get().resetMessages?.(language); // chatSlice 호출
      get().setIsLoading?.(false); // 로딩 상태 해제
      return null;
  ...
  deleteConversation: async (conversationId) => {
    ...
    if (!user || typeof conversationId !== 'string' || !conversationId) {
        if (typeof conversationId !== 'string' || !conversationId) console.error("deleteConversation invalid ID:", conversationId);
        return;
     }
  ...
  updateConversationTitle: async (conversationId, newTitle) => {
    ...
    if (!user || typeof conversationId !== 'string' || !conversationId || typeof newTitle !== 'string' || !newTitle.trim()) {
        if (typeof newTitle !== 'string' || !newTitle.trim()) showEphemeralToast("Title cannot be empty.", "error");
        return;
    }
  ...
  pinConversation: async (conversationId, pinned) => {
     const { user, language, showEphemeralToast } = get();
     if (!user || typeof conversationId !== 'string' || !conversationId || typeof pinned !== 'boolean') return;
    try {
        const conversationRef = doc( get().db, "chats", user.uid, "conversations", conversationId );
  ...
  toggleConversationExpansion: (conversationId) => {
    const { expandedConversationId, /* unsubscribeScenariosMap 제거 */ user, language, showEphemeralToast } = get();
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      set((state) => ({ scenariosForConversation: { ...state.scenariosForConversation, [conversationId]: scenarios } }));
      ...
      // });
        ...
        set((state) => ({
            ...(state.expandedConversationId === conversationId ? { expandedConversationId: null } : {}),
            // unsubscribeScenariosMap는 scenarioSlice에서 관리하므로 여기서 직접 건드리지 않음
            scenariosForConversation: { ...state.scenariosForConversation, [conversationId]: [] },
        }));
